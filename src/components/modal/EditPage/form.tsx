...
const CreatNewPageForm = ({ setModalOpen, refetch }: Iprops) => {
  ...
  const { currentApp, currentPage } = useStore((store: any) => store);

  const form = useForm<z.infer<typeof FormSchema>>({
    resolver: zodResolver(FormSchema),
    defaultValues: {
      name: currentPage?.title,
      website_url: currentPage?.url,
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    try {
      const res = await API.put(`/pages/${currentPage?.id}?app_id=${currentApp?.data?.id}`, {
        title: data.name,
      ...
      setMessage({ text: 'Page Edited', isError: false });
      refetch();
      setTimeout(() => {
        setModalOpen(false);
      }, 1000);
            ...
            <div className=' flex flex-col items-center gap-4'>
              ...
              >
                <span className='font-[500] leading-[1.5rem] tracking-[0.02875rem] text-white disabled:cursor-not-allowed disabled:opacity-50'>
                  {formIsLoading ? <Spinner /> : 'Edit Page'}
                </span>
              </button>
