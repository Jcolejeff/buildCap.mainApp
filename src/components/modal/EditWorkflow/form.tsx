...
const CreatNewPageForm = ({ setModalOpen, refetch }: Iprops) => {
  ...
  const form = useForm<z.infer<typeof FormSchema>>({
    ...
    defaultValues: {
      ...
      website_url: currentWorkflow?.url,
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    try {
      const res = await API.put(`/workflows/${currentWorkflow?.id}?page_id=${pageId}`, {
        is_active: true,
        name: data.name,
      ...
      setMessage({ text: 'Workflow edited', isError: false });
      refetch();
      setTimeout(() => {
        setModalOpen(false);
      }, 1000);
            ...
            <div className=' flex flex-col items-center gap-4'>
              ...
              >
                <span className='font-[500] leading-[1.5rem] tracking-[0.02875rem] text-white disabled:cursor-not-allowed disabled:opacity-50'>
                  {formIsLoading ? <Spinner /> : 'Edit Workflow'}
                </span>
              </button>
