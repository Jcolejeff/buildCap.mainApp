...
const CreateNewPage = ({ trigger, triggerClassName, title, refetch }: Iprop) => {
  const [modalOpen, setModalOpen] = useState(false);

  return (
    <Dialog onOpenChange={(i) => setModalOpen(i)} open={modalOpen}>
      <DialogTrigger className={triggerClassName}>{trigger}</DialogTrigger>
      <DialogContent className='no-scrollbar  max-w-full overflow-auto overflow-x-hidden  bg-white  px-6 md:w-fit md:!max-w-[1000px] lg:px-[2rem]'>
